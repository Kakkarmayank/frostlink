...
# ---------------------------
# When this file is executed directly (e.g. `streamlit run frostlink/config.py`)
# we want to render the interactive Streamlit UI. If streamlit isn't available
# or import fails, fall back to the launcher behavior.
if __name__ == "__main__":
    try:
        # If Streamlit is running this script, the `streamlit` package will be importable
        # and we can call the UI function directly so the page renders.
        import streamlit  # noqa: F401
        _interactive_setup_ui()
    except Exception:
        # If anything goes wrong (e.g. streamlit not installed), fall back to the existing launcher
        main()
